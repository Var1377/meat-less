...
>
	...
	<section class="flex gap-4 flex-wrap">
		{#if $barcodeStore?.product?.tokens?.MeatlessToken}
			<p class="chip variant-glass-primary">{$barcodeStore.product.tokens.MeatlessToken} MeatlessToken</p>
		{/if}
		{#if $barcodeStore?.product?.tokens?.LocalToken}
			<p class="chip variant-glass-secondary">{$barcodeStore.product.tokens.LocalToken} LocalToken</p>
		{/if}
	</section>
